...
KiCode Desktop runs a local OpenCode server (the `opencode-cli` sidecar) in the background. Most issues are caused by a misbehaving plugin, a corrupted cache, or a bad server setting.
...
1. Quit KiCode Desktop completely.
...
3. Restart KiCode Desktop.

---

### Fix server connection issues

KiCode Desktop can either start its own local server (default) or connect to a server URL you configured.
...
On Windows, KiCode Desktop requires the Microsoft Edge **WebView2 Runtime**. If the app opens to a blank window or won't start, install/update WebView2 and try again.
...
KiCode Desktop only shows system notifications when:
...
1. Quit KiCode Desktop.
2. Find and delete these files (they live in the KiCode Desktop app data directory):
