...
KiCode Desktop kører en lokal OpenCode-server (`opencode-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.
...
3. Start KiCode Desktop på ny.

---

### Løs problemer med serverforbindelse

KiCode Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
På Windows kræver KiCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.
...
KiCode Desktop viser kun systemvarsler når:
...
1. Afslut KiCode Desktop.
2. Find og slet disse filer (de findes i KiCode Desktop-appens datamappe):
