    ...
    await Instance.provide({
      ...
      async fn() {
        ...
        if (oauthServers.length === 0) {
          ...
          prompts.log.info("Remote MCP servers support OAuth by default. Add a remote server in kicode.json:")
          prompts.log.info(`
  "mcp": {
    "my-server": {
      "type": "remote",
...
async function resolveConfigPath(baseDir: string, global = false) {
  // Check for existing config files (prefer .jsonc over .json, check .kicode/ subdirectory too)
  const candidates = [path.join(baseDir, "kicode.json"), path.join(baseDir, "kicode.jsonc")]

  if (!global) {
    candidates.push(path.join(baseDir, ".kicode", "kicode.json"), path.join(baseDir, ".kicode", "kicode.jsonc"))
  }
  ...
  // Default to kicode.json if none exist
  return candidates[0]
}
