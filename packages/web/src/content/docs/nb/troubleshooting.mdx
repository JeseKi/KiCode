...
KiCode Desktop kjører en lokal OpenCode-server (`opencode-cli` sidevognen) i bakgrunnen. De fleste problemene er forårsaket av en plugin som fungerer dårlig, en ødelagt cache eller en dårlig serverinnstilling.
...
3. Start KiCode Desktop på nytt.

---

### Løs problemer med servertilkobling

KiCode Desktop kan enten starte sin egen lokale server (standard) eller koble til en server URL du har konfigurert.
...
På Windows krever KiCode Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åpnes i et tomt vindu eller ikke starter, installer/oppdater WebView2 og prøv igjen.
...
KiCode Desktop viser bare systemvarsler når:
...
1. Avslutt KiCode Desktop.
2. Finn og slett disse filene (de finnes i KiCode Desktop-appens datakatalog):
