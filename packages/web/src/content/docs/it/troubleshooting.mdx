...
KiCode Desktop esegue in background un server locale di OpenCode (il sidecar `opencode-cli`). La maggior parte dei problemi e causata da un plugin che si comporta male, da una cache corrotta o da un'impostazione del server errata.
...
1. Chiudi completamente KiCode Desktop.
...
3. Riavvia KiCode Desktop.

---

### Risolvi problemi di connessione al server

KiCode Desktop puo avviare il proprio server locale (predefinito) oppure connettersi a un URL server che hai configurato.
...
Su Windows, KiCode Desktop richiede **WebView2 Runtime** di Microsoft Edge. Se l'app si apre su una finestra vuota o non parte, installa/aggiorna WebView2 e riprova.
...
KiCode Desktop mostra le notifiche di sistema solo quando:
...
1. Chiudi KiCode Desktop.
2. Trova ed elimina questi file (si trovano nella directory dati dell'app KiCode Desktop):
