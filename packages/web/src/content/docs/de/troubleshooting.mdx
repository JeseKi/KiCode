...
KiCode Desktop startet im Hintergrund einen lokalen OpenCode-Server (`opencode-cli`-Sidecar).
...
1. Beende KiCode Desktop komplett.
...
3. Starte KiCode Desktop neu.

---

### Server-Verbindungsprobleme beheben

KiCode Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Unter Windows benoetigt KiCode Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.
...
KiCode Desktop zeigt Systembenachrichtigungen nur wenn:
...
1. Beende KiCode Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von KiCode Desktop):
