...
KiCode Desktop 會在背景執行一個本地 OpenCode 伺服器（即 `opencode-cli` 附屬程序）。大多數問題是由外掛異常、快取損壞或錯誤的伺服器設定引起的。
...
3. 重新啟動 KiCode Desktop。

---

### 修復伺服器連線問題

KiCode Desktop 可以啟動自己的本地伺服器（預設行為），也可以連線到你設定的伺服器 URL。
...
在 Windows 上，KiCode Desktop 需要 Microsoft Edge **WebView2 Runtime**。如果應用程式開啟後是空白視窗或無法啟動，請安裝或更新 WebView2 後重試。
...
1. 退出 KiCode Desktop。
2. 找到並刪除以下檔案（它們位於 KiCode Desktop 應用程式資料目錄中）：
