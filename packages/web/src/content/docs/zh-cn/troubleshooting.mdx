...
KiCode Desktop 会在后台运行一个本地 OpenCode 服务器（即 `opencode-cli` 附属进程）。大多数问题是由插件异常、缓存损坏或错误的服务器设置引起的。
...
3. 重新启动 KiCode Desktop。

---

### 修复服务器连接问题

KiCode Desktop 可以启动自己的本地服务器（默认行为），也可以连接到你配置的服务器 URL。
...
在 Windows 上，KiCode Desktop 需要 Microsoft Edge **WebView2 Runtime**。如果应用打开后是空白窗口或无法启动，请安装或更新 WebView2 后重试。
...
1. 退出 KiCode Desktop。
2. 找到并删除以下文件（它们位于 KiCode Desktop 应用数据目录中）：
