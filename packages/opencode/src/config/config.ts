...
export namespace Config {
  ...
  function systemManagedConfigDir(): string {
    switch (process.platform) {
      case "darwin":
        return "/Library/Application Support/kicode"
      case "win32":
        return path.join(process.env.ProgramData || "C:\\ProgramData", "kicode")
      default:
        return "/etc/kicode"
    }
  }
  ...
  async function loadCommand(dir: string) {
    ...
    for (const item of await Glob.scan("{command,commands}/**/*.md", {
      ...
      const patterns = ["/.kicode/command/", "/.kicode/commands/", "/command/", "/commands/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for (const item of await Glob.scan("{agent,agents}/**/*.md", {
      ...
      const patterns = ["/.kicode/agent/", "/.kicode/agents/", "/agent/", "/agents/"]
      const file = rel(item, patterns) ?? path.basename(item)
    ...
    .preprocess(
      ...
      z
        .object({
          ...
          imagegen: PermissionAction.optional(),
  ...
  function globalConfigFile() {
    const candidates = ["kicode.jsonc", "kicode.json", "config.json"].map((file) =>
      ...
      Effect.gen(function* () {
        ...
        const loadGlobal = Effect.fnUntraced(function* () {
          let result: Info = pipe(
            ...
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "kicode.json"))),
            mergeDeep(yield* loadFile(path.join(Global.Path.config, "kicode.jsonc"))),
          )
        ...
        const loadInstanceState = Effect.fnUntraced(function* (ctx: InstanceContext) {
          ...
          if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
            for (const file of yield* Effect.promise(() =>
              ConfigPaths.projectFiles("kicode", ctx.directory, ctx.worktree),
            )) {
          ...
          for (const dir of unique(directories)) {
            if (dir.endsWith(".kicode") || dir === Flag.OPENCODE_CONFIG_DIR) {
              for (const file of ["kicode.jsonc", "kicode.json"]) {
